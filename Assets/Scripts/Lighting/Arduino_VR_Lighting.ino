...
void checkEventExpiration() {
  ...
  case EVENT_COL:
    ...
    break;
  case EVENT_BST:
    duration = BST_DURATION;
    break;
  case EVENT_DNG:
    duration = DNG_DURATION;
    break;
  case EVENT_WIN:
    duration = WIN_DURATION;
    break;
  case EVENT_RNG:
    duration = RNG_DURATION;
    break;
  }
...
void updateLEDs() {
  ...
  if (isTimerActive) {
    ...
    if (secondsPassed != lastSecondsLogged) {
      if (secondsPassed == 15)
        Serial.println(F("[TIMER] Remaining: 45"));
      else if (secondsPassed == 30)
        Serial.println(F("[TIMER] Remaining: 30"));
      else if (secondsPassed == 50)
        Serial.println(F("[TIMER] Remaining: 10"));
    ...
    if (secondsPassed >= timerDurationSec) {
      ...
      if (remainingSeconds <= 20) {
        // Sync flash cycle to start predictably ON exactly when urgent mode
        // begins (40,000 ms elapsed)
        unsigned long urgentElapsed =
            elapsedTimer - ((timerDurationSec - 20) * 1000UL);
...
void runRingEffect() {
  ...
  if (elapsed < 200) {
    ...
    if (pos >= 0 && pos < MAIN_NUM_LEDS) {
      ...
      // Draw a few extra pixels for a thicker head, trailing backwards (higher
      // indices)
      if (pos + 1 < MAIN_NUM_LEDS)
        mainLeds[pos + 1] = ringColor;
      if (pos + 2 < MAIN_NUM_LEDS)
        mainLeds[pos + 2] = CRGB(255, 140, 0); // Amber/Gold mix
      if (pos + 3 < MAIN_NUM_LEDS)
        mainLeds[pos + 3] = CRGB(200, 100, 0);
    }
  } else {
